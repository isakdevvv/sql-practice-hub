...
import { PAKKE_PREDICT_ITEMS } from "@/lib/dte2505/pakkekilderAnslag";

// ---------------------------------------------------------------------------
// Oppgavetype 1 — ANSLÅ-SÅ-SJEKK, plassert FØR forklaringene på siden.
// Ingen poengsum: et anslag som bommer gjør nettopp jobben sin.
// ---------------------------------------------------------------------------

export function PakkeAnsla() {
  const [idx, setIdx] = useState(0);
  const [guesses, setGuesses] = useState<Record<string, string>>({});

  const item = PAKKE_PREDICT_ITEMS[idx];
    ...
    <div className="rounded-xl border-2 border-brand/30 bg-card">
      <div className="flex items-center justify-between border-b px-4 py-2.5">
        <div className="flex items-center gap-2 text-sm font-semibold">
          ...
          <span className="text-xs font-normal text-muted-foreground">
            {idx + 1} / {PAKKE_PREDICT_ITEMS.length}
          </span>
        </div>
      ...
      <div className="p-4">
        ...
        <div className="mt-4 flex items-center justify-between border-t pt-3">
          <button
            onClick={() => setIdx((i) => (i - 1 + PAKKE_PREDICT_ITEMS.length) % PAKKE_PREDICT_ITEMS.length)}
          ...
          <button
            onClick={() => setIdx((i) => (i + 1) % PAKKE_PREDICT_ITEMS.length)}
