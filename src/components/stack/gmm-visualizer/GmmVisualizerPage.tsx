      ...
      <main className="container mx-auto px-4 py-8 max-w-4xl">
        <header className="mb-6">
          ...
          <p className="mt-1 text-sm text-muted-foreground max-w-2xl">
            Géron kap. 8, MML kap. 11. Hvordan EM-algoritmen finner sannsynlige klynger uten å vite
            hvilken klynge hvert punkt tilhører. Bygger på dte2501-kmeans-visualizer og dte2501-gmm.
          </p>
        </header>
    ...
    <div className="space-y-4 text-sm">
      <div className="rounded-xl border border-border bg-card p-4">
        ...
        <ul className="list-disc pl-5 text-muted-foreground space-y-1">
          <li>
            <strong className="text-foreground">k-Means</strong> (
            <code>dte2501-kmeans-visualizer</code>): har sett hard assignment — hvert punkt tilhører
            nøyaktig én klynge.
          </li>
          <li>
            <strong className="text-foreground">Normalfordeling</strong> (
            <code>tek1-kontinuerlige-fordelinger</code>): gjennomsnitt μ, varians σ². Den
            klokke-formede fordelingen.
          </li>
        </ul>
      </div>
      ...
      <div className="rounded-xl border border-border bg-card p-4">
        ...
        <p className="text-muted-foreground">
          k-Means antar at klynger er sirkulære og like store. Hvis de IKKE er det — f.eks. ene
          klynga er lang og smal, og en annen er bred — gir k-Means feil grupper.
        </p>
        <p className="text-muted-foreground mt-2">
          GMM antar at hvert punkt er trukket fra en av K normalfordelinger. Hver fordeling har sin
          egen μ (sentrum) og Σ (kovariansmatrise = form). Punkter tilhører klynger med{" "}
          <em>sannsynlighet</em> — ikke binært.
        </p>
      </div>
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-lg border border-border bg-muted/30 p-3 text-xs text-muted-foreground">
        <strong className="text-foreground">3 klynger</strong> (sirkulær, lang horisontal ellipse,
        lang vertikal ellipse). Hver punkt er farget etter hvilken komponent som har høyest
        «responsibility» — fargen er en blanding hvis flere komponenter konkurrerer. Trykk «Step» én
        gang per EM-iterasjon, eller «Auto» for kontinuerlig kjøring.
      </div>

      <div className="rounded-xl border border-border bg-card p-4">
        <div className="flex gap-1.5 mb-3 items-center">
          <Button size="sm" onClick={emStep}>
            EM-step
          </Button>
          <Button
            size="sm"
        ...
        <svg viewBox="0 0 450 350" className="w-full h-auto bg-muted/20 rounded">
          ...
          {data.map((p, i) => {
            ...
            return (
              <circle key={i} cx={p.x} cy={p.y} r={3} className={cls} opacity={0.3 + conf * 0.7} />
            );
          })}
          {/* Sentre */}
          {components.map((c, k) => (
        ...
        <div className="mt-3 grid gap-2 sm:grid-cols-3 text-xs">
          {components.map((c, k) => (
            <div
              key={k}
              className={`rounded border p-2 ${k === 0 ? "border-brand/40 bg-brand/5" : k === 1 ? "border-amber-500/40 bg-amber-500/5" : "border-success/40 bg-success/5"}`}
            >
              <div className="font-semibold">Komponent {k + 1}</div>
              <div className="font-mono text-[10px] text-muted-foreground">
                π = {c.pi.toFixed(2)}
                <br />μ = ({c.mu.x.toFixed(0)}, {c.mu.y.toFixed(0)})<br />σ = (
                {c.sigma.sx.toFixed(0)}, {c.sigma.sy.toFixed(0)})
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="mt-3 text-xs text-muted-foreground">
          Etter ~10-20 EM-iterasjoner stabiliserer ellipsene seg over de 3 sanne klyngene. Legg
          merke til at den langstrakte horisontale klyngen får σ_x &gt;&gt; σ_y — noe k-Means aldri
          kunne fanget.
        </div>
      </div>
    </div>
  ...
  return (
    <section className="mt-10 space-y-3 text-sm">
      ...
      <ul className="list-disc pl-5 space-y-1.5 text-muted-foreground">
        <li>
          <strong className="text-foreground">GMM = soft clustering</strong>. I motsetning til
          k-Means gir den probabilistiske tilhørighet. Bra når klynger overlapper.
        </li>
        <li>
          <strong className="text-foreground">EM-garanti:</strong> log-likelihood ØKER monotont per
          iterasjon (eller står stille). Garanterer ikke globalt optimum — sensitive for
          initialisering.
        </li>
        <li>
          <strong className="text-foreground">Valg av K:</strong> bruk BIC eller AIC
          (informasjons-kriterier som straffer kompleksitet). For mange komponenter overtilpasser;
          for få mister struktur.
        </li>
        <li>
          <strong className="text-foreground">Anvendelser:</strong> bakgrunns-subtraksjon i video,
          speaker identification, anomali-deteksjon (data utenfor høy-likelihood-områder), generativ
          modellering (sample fra modellen for å lage syntetisk data).
        </li>
      </ul>
    </section>
  );
