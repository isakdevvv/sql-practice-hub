      ...
      <div className="flex flex-wrap items-center gap-4 text-xs">
        ...
        <label className="flex items-center gap-2 cursor-pointer">
          <input
            type="checkbox"
            checked={twoTests}
            onChange={(e) => setTwoTests(e.target.checked)}
          />
        ...
        <div className="rounded-lg border border-border bg-background p-4">
          ...
          <div className="h-3 mt-2 rounded-full bg-muted overflow-hidden border border-border">
            <div
              ...
              style={{ width: `${calc.pDGivenPos * 100}%` }}
            />
          </div>
          <div className="text-[11px] text-muted-foreground mt-3 font-mono leading-relaxed">
            P(+) = {calc.sens.toFixed(3)}·{calc.pD.toFixed(3)} + {calc.fpr.toFixed(3)}·
            {calc.pNotD.toFixed(3)}
            <br />
            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; = {(calc.sens * calc.pD).toFixed(5)} +{" "}
            {(calc.fpr * calc.pNotD).toFixed(5)}
            <br />
            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; = {calc.pPos.toFixed(5)}
            <br />
            P(D|+) = {(calc.sens * calc.pD).toFixed(5)} / {calc.pPos.toFixed(5)} ={" "}
            {calc.pDGivenPos.toFixed(4)}
          </div>
        </div>

        <div
          className={`rounded-lg border p-4 transition-opacity ${twoTests ? "border-border bg-background" : "border-dashed border-border bg-muted/20 opacity-60"}`}
        >
          ...
          <div className="h-3 mt-2 rounded-full bg-muted overflow-hidden border border-border">
            <div
              className="h-full bg-rose-500 transition-all"
              style={{ width: `${(twoTests ? calc.pDGivenPP : 0) * 100}%` }}
            />
          </div>
          <div className="text-[11px] text-muted-foreground mt-3 leading-relaxed">
            Antar betinget uavhengighet gitt klasse (samme test 2 ganger gir korrelert feil, så
            dette er ofte optimistisk). To uavhengige tester er en raskere måte å nå høy konfidens
            enn å forbedre én enkelt test.
          </div>
        </div>
      </div>
      ...
      {showTable && (
        <div>
          <div className="text-xs font-semibold mb-2">
            1000 personer trekkes tilfeldig fra populasjonen
          </div>
              ...
              <tbody>
                ...
                <tr className="border-t border-border bg-muted/30 font-semibold">
                  ...
                  <td className="px-3 py-1.5 text-right tabular-nums">
                    {(calc.sickPos + calc.healthyPos).toFixed(1)}
                  </td>
                  <td className="px-3 py-1.5 text-right tabular-nums">
                    {(calc.sickNeg + calc.healthyNeg).toFixed(1)}
                  </td>
          ...
          <p className="text-[11px] text-muted-foreground mt-2">
            Av {(calc.sickPos + calc.healthyPos).toFixed(0)} positive tester er bare{" "}
            {calc.sickPos.toFixed(0)} faktisk syke ⇒ P(D|+) = {calc.sickPos.toFixed(1)} /{" "}
            {(calc.sickPos + calc.healthyPos).toFixed(1)} ≈ {(calc.pDGivenPos * 100).toFixed(1)} %.
          </p>
        </div>
      )}

      <div className="rounded-md border border-brand/30 bg-brand/5 p-3 text-[11px] leading-relaxed">
        <strong>Prøv:</strong> sett prevalens til 1% og sensitivitet/spesifisitet til 99% hver.
        Posterior blir kun ~50%. Skru prevalensen opp til 10% — nå klatrer posterior til ~92%.
        Bayes' moral: prior dominerer når sykdommen er sjelden, uansett hvor god testen er.
      </div>
    </div>
  );
}
