      ...
      <main className="container mx-auto px-4 py-8 max-w-4xl">
        <header className="mb-6">
          ...
          <p className="mt-1 text-sm text-muted-foreground max-w-2xl">
            Kurose kap. 7.2. Hvorfor WiFi <em>ikke</em> kan kollisjons-detektere
            som kablet Ethernet — og hvordan CSMA/CA + RTS/CTS løser
            «hidden terminal»-problemet. Bygger på dte2507-aloha-kasino.
          </p>
        </header>
        <div className="mb-4 flex flex-wrap gap-1.5 border-b border-border">
          ...
          <TabBtn active={tab === "live"} onClick={() => setTab("live")} icon={<Wifi className="h-3.5 w-3.5" />}>1. CSMA/CD vs CSMA/CA</TabBtn>
        </div>
    ...
    <div className="space-y-4 text-sm">
      <div className="rounded-xl border border-border bg-card p-4">
        ...
        <ul className="list-disc pl-5 text-muted-foreground space-y-1">
          <li><strong className="text-foreground">ALOHA & CSMA/CD</strong> (<code>dte2507-aloha-kasino</code>): «slot ALOHA», «sense before sending», kollisjons-deteksjon med backoff.</li>
          <li><strong className="text-foreground">MAC-laget</strong>: rammer på linklaget, MAC-adresser, broadcast-domener.</li>
        </ul>
      </div>

      <div className="rounded-xl border border-border bg-card p-4">
        <h2 className="text-base font-semibold mb-2">Hvorfor WiFi ikke kan «collision detection»</h2>
        <p className="text-muted-foreground">
          I kablet Ethernet kan en sender LYTTE på kabelen mens den sender. Hvis to noder sender samtidig, ser de begge en forvrengt signal og <em>vet</em> at det er kollisjon. Det heter CSMA/CD (Collision Detection).
        </p>
        <p className="text-muted-foreground mt-2">
          WiFi kan ikke det. Radio er halv-duplex: en sender kan ikke lytte på sitt EGET sterke signal samtidig som den sender. I tillegg er det <em>hidden terminal</em>-problemet: A kan høre AP-en, og C kan høre AP-en, men A og C kan IKKE høre hverandre. De vet ikke om den andre sender.
        </p>
        <p className="text-muted-foreground mt-2">
          <strong className="text-foreground">Løsningen: CSMA/CA</strong> (Collision Avoidance). I stedet for å detektere kollisjoner prøver vi å unngå dem. Pluss RTS/CTS som «reservasjon-protokoll» for store rammer.
        </p>
      </div>
      ...
      <div className="rounded-xl border border-border bg-card p-4">
        ...
        <dl className="space-y-2.5 text-[13px]">
          <Def term="CSMA">Carrier Sense Multiple Access. «Lytt før du sender». Alle WiFi-noder venter til luften er ledig før de prøver å sende.</Def>
          <Def term="CSMA/CD vs CSMA/CA">CD = Collision Detection (kablet Ethernet). CA = Collision Avoidance (WiFi). Forskjellen: CD reagerer på kollisjoner mens de skjer; CA forsøker å forhindre dem.</Def>
          <Def term="DIFS / SIFS">Distributed/Short Inter-Frame Space. Stille-perioder med faste lengder:
            <ul className="list-disc pl-5 mt-1">
              <li><strong>SIFS</strong> (~10 µs): kort, brukt mellom RTS/CTS/DATA/ACK i samme utveksling.</li>
              <li><strong>DIFS</strong> (~50 µs): lengre, krevd FØR en helt ny ramme kan sendes.</li>
            </ul>
          </Def>
          <Def term="Backoff (random)">Etter DIFS venter senderen ekstra tilfeldig tid (et tall av slots). To noder som vil sende samtidig får dermed ulik ventetid og kolliderer ikke. Kollisjon → backoff dobles (exponential backoff).</Def>
          <Def term="RTS / CTS">Request to Send / Clear to Send. Små kontroll-rammer:
            <ul className="list-disc pl-5 mt-1">
          ...
          <Def term="Hidden terminal">A og C er begge innenfor AP-ens rekkevidde, men ikke innenfor hverandres. Uten RTS/CTS kan de begge tenke «luften er ledig» samtidig og kollidere på AP-en. RTS/CTS fikser dette fordi AP-ens CTS når begge.</Def>
          <Def term="ACK">Etter at en ramme er mottatt riktig sender mottakeren en ACK-ramme. Hvis senderen ikke ser ACK innen et timeout, antar den kollisjon og prøver på nytt med dobbel backoff.</Def>
        </dl>
      </div>
...
type Event = { t: number; node: string; type: "DIFS" | "BACKOFF" | "RTS" | "CTS" | "DATA" | "ACK" | "SIFS" | "JAM"; len: number; color: string };
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-lg border border-border bg-muted/30 p-3 text-xs text-muted-foreground">
        <strong className="text-foreground">Hva skjer:</strong> to noder A og C vil sende til AP-en samtidig.
        I scenario «hidden» kan A og C ikke høre hverandre direkte. Se hvordan
        hver modell håndterer situasjonen — særlig hvor RTS/CTS endrer alt.
      </div>

      <div className="rounded-xl border border-border bg-card p-4">
        <div className="flex flex-wrap gap-2 mb-3">
          <div className="text-xs text-muted-foreground self-center">Modell:</div>
          <Button size="sm" variant={mode === "cd" ? "default" : "outline"} onClick={() => setMode("cd")}>CSMA/CD (Ethernet)</Button>
          <Button size="sm" variant={mode === "ca" ? "default" : "outline"} onClick={() => setMode("ca")}>CSMA/CA (WiFi, uten RTS)</Button>
          <Button size="sm" variant={mode === "ca-rtscts" ? "default" : "outline"} onClick={() => setMode("ca-rtscts")}>CSMA/CA + RTS/CTS</Button>
        </div>
        <div className="flex flex-wrap gap-2 mb-3">
          <div className="text-xs text-muted-foreground self-center">Scenario:</div>
          <Button size="sm" variant={scenario === "none" ? "default" : "outline"} onClick={() => setScenario("none")}>A og C ser hverandre</Button>
          <Button size="sm" variant={scenario === "hidden" ? "default" : "outline"} onClick={() => setScenario("hidden")}>Hidden terminal (A og C ser IKKE hverandre)</Button>
        </div>
          ...
          {["A", "C", "AP"].map((node) => (
            <div key={node} className="flex items-center gap-2">
              ...
              <div className="flex-1 h-7 rounded bg-muted relative overflow-hidden">
                {events.filter((e) => e.node === node).map((e, i) => (
                  <div key={i}
...
function buildScenario(mode: "cd" | "ca" | "ca-rtscts", scenario: "none" | "hidden"): Event[] {
  const ev: Event[] = [];
  const cAmber = "bg-amber-500", cBrand = "bg-brand", cSuccess = "bg-success", cDestructive = "bg-destructive";
  ...
  return (
    <section className="mt-10 space-y-3 text-sm">
      ...
      <ul className="list-disc pl-5 space-y-1.5 text-muted-foreground">
        <li><strong className="text-foreground">CSMA/CD ble dropt for WiFi</strong> fordi radio-noder ikke kan lytte mens de sender, og fordi hidden terminals gjør detection upålitelig.</li>
        <li><strong className="text-foreground">CSMA/CA er fundamentet:</strong> lytt, vent DIFS, tilfeldig backoff, send, vent ACK. Exponential backoff ved feil.</li>
        <li><strong className="text-foreground">RTS/CTS</strong> er valgfri optimalisering for store rammer i miljøer med hidden terminals. Slått av som default i moderne WiFi (802.11ac+) fordi små noder gjør den unødvendig.</li>
        <li><strong className="text-foreground">Effektivitet:</strong> WiFi tåler mye lavere utnyttelse enn Ethernet. 54 Mbps 802.11g gir ofte 20-25 Mbps reell throughput pga. backoff, SIFS, ACK.</li>
      </ul>
    </section>
  );
}
