...
import { ChevronLeft, ChevronRight, RotateCcw, Sparkles, Target, Package, ShieldQuestion } from "lucide-react";

export function GreedyNpVizPage() {
  const [tab, setTab] = useState<"setcover" | "tsp" | "knapsack" | "pvsnp">("setcover");
      ...
      <main className="container mx-auto px-4 py-8 max-w-4xl">
        <header className="mb-6">
          ...
          <p className="mt-1 text-sm text-muted-foreground max-w-2xl">
            Fire moduler som viser hvorfor greedy-approksimasjon er den
            praktiske utveien for NP-harde problemer: set cover, TSP, knapsack
            og P-vs-NP-intuisjonen.
          </p>
        </header>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        ...
        <div className="rounded-xl border border-border bg-card p-3 text-xs">
          ...
          <div className="space-y-1.5">
            ...
            <Row label="Optimum (brute-force 2⁶)" value={`${optimum.count} stasjoner (${optimum.sets.join(", ")})`} />
            <div className="text-[11px] text-muted-foreground pt-1">
              Greedy garanterer ≤ <code>H<sub>n</sub></code> ≈ ln(n) ganger
              optimum. Det er ingen kjent polynom-algoritme som gjør det
              bedre — set cover er NP-hardt.
            </div>
          </div>
        </div>
      </div>
    ...
    <div>
      <div className="rounded-xl border border-border bg-card overflow-hidden">
        <div className="bg-muted/30 px-4 py-2 text-xs text-muted-foreground border-b border-border flex justify-between items-center">
          <span>
            Nærmeste-nabo (blå) starter på by 0, tar alltid nærmeste ubesøkte
            by. Brute-force optimum (grønn) prøver alle (n−1)! permutasjoner.
          </span>
    ...
    <div>
      <div className="rounded-xl border border-border bg-card p-4">
        <div className="text-sm mb-3">
          <strong>Subset Sum</strong> (NP-komplett). Finn en delmengde av tallene som summerer til{" "}
...
function Lessons() {
  return (
    <section className="mt-10 space-y-3 text-sm">
      <h2 className="text-lg font-semibold">Hva Grokking kap. 10 prøver å lære deg</h2>
      <ul className="list-disc pl-5 space-y-1.5 text-muted-foreground">
        <li>
          <strong className="text-foreground">Greedy</strong> = «velg alltid det som ser
          best ut akkurat nå». Rask, men ikke alltid optimal.
        </li>
        <li>
          <strong className="text-foreground">NP-komplett</strong> = en klasse problemer
          der vi ikke kjenner noen polynom-algoritme. Hvis du finner én for ETT av dem
          (set cover, TSP, subset sum, SAT, ...), så har du løst dem alle.
        </li>
        <li>
          <strong className="text-foreground">Approksimasjon</strong> er praksis: for
          set cover gir greedy garantert ≤ ln(n) ganger optimum. For TSP med trekant-
          ulikheten finnes 1.5-approksimasjon (Christofides). For knapsack finnes
          en FPTAS som kommer vilkårlig nær.
        </li>
        <li>
          <strong className="text-foreground">Når du møter et problem som ligner</strong>{" "}
          (set cover-aktig, schedulering, ressursvelging) — sjekk om det er kjent
          NP-hardt. Da slipper du å lete etter en perfekt algoritme som ikke finnes.
        </li>
      </ul>
    </section>
  );
