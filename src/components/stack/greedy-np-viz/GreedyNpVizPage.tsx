...
import { ChevronLeft, ChevronRight, RotateCcw, BookOpen, Sparkles, Target, Package, ShieldQuestion } from "lucide-react";
      ...
      <main className="container mx-auto px-4 py-8 max-w-4xl">
        <header className="mb-6">
          ...
          <p className="mt-1 text-sm text-muted-foreground max-w-2xl">
            Fem trinn. Start på «0. Start her» — der introduseres hvert ord og
            hvert symbol du møter senere. De fire neste er hands-on moduler.
          </p>
        </header>
        ...
        <div className="mb-4 flex flex-wrap gap-1.5 border-b border-border">
          ...
          <TabBtn active={tab === "setcover"} onClick={() => setTab("setcover")} icon={<Sparkles className="h-3.5 w-3.5" />}>
            1. Set cover
          </TabBtn>
          <TabBtn active={tab === "tsp"} onClick={() => setTab("tsp")} icon={<Target className="h-3.5 w-3.5" />}>
            2. TSP
          </TabBtn>
          <TabBtn active={tab === "knapsack"} onClick={() => setTab("knapsack")} icon={<Package className="h-3.5 w-3.5" />}>
            3. Knapsack
          </TabBtn>
          <TabBtn active={tab === "pvsnp"} onClick={() => setTab("pvsnp")} icon={<ShieldQuestion className="h-3.5 w-3.5" />}>
            4. P vs NP
          </TabBtn>
        </div>
    ...
    <div className="space-y-4 text-sm">
      <div className="rounded-xl border border-border bg-card p-4">
        ...
        <dl className="space-y-3 text-[13px]">
          <Def term="Algoritme">
            En liste oppskrifter datamaskinen følger for å løse et problem.
            «Sorter denne lista» eller «finn kortest vei».
          </Def>
          <Def term="Greedy (grådig)">
            En algoritme-strategi: i hvert steg, ta valget som ser best ut
            <em> akkurat nå</em>, uten å tenke på fremtiden. Rask, men ikke
            alltid optimal.
          </Def>
          <Def term="Optimum / optimal løsning">
            Det beste mulige svaret. F.eks. det kortest mulige TSP-toget gjennom
            alle byer. Greedy gir ofte <em>ikke</em> optimum, men noe nær.
          </Def>
          <Def term="Brute force">
            «Prøv absolutt alle muligheter og velg det beste». Garantert
            optimum, men eksplosivt tregt når problemet vokser.
          </Def>
          <Def term="n (kursivt n)">
            Antall input-elementer. Hvis du sorterer 100 tall er <code>n = 100</code>.
            Vi måler hvor mange operasjoner algoritmen bruker som funksjon av n.
          </Def>
          <Def term="O(...) — Big-O">
            En «hastighets-merkelapp» som forteller hvor mange operasjoner i
            verste fall. Eksempler:
            <ul className="mt-1 list-disc pl-5">
              <li><code>O(n)</code> = vokser proporsjonalt med n. 100 elementer → 100 operasjoner.</li>
              <li><code>O(n²)</code> = vokser med n²: 100 elementer → 10 000 operasjoner.</li>
              <li><code>O(n log n)</code> = litt mer enn lineær. God sortering.</li>
              <li><code>O(2ⁿ)</code> = eksponentiell. 30 elementer → 1 milliard operasjoner.</li>
              <li><code>O(n!)</code> = «n fakultet» = 1·2·3·...·n. 10! = 3.6 millioner. 20! = 2.4 trillioner.</li>
            </ul>
          </Def>
          <Def term="Polynom tid">
            Algoritmer hvis kjøretid er <code>O(n^k)</code> for et fast tall k:
            <code>O(n)</code>, <code>O(n²)</code>, <code>O(n³)</code> osv. «Raskt
            nok» til at det skalerer i praksis.
          </Def>
          <Def term="P">
            Mengden problemer du kan løse i polynom tid. Sortering, søk,
            korteste vei (Dijkstra) er i P.
          </Def>
          <Def term="NP">
            Mengden problemer du kan <em>verifisere</em> en gitt løsning på i
            polynom tid. («Verifisere» betyr sjekke at svaret stemmer.) Alle
            P-problemer er i NP, men NP har også problemer vi ikke vet hvordan
            å løse raskt.
          </Def>
          <Def term="NP-hardt / NP-komplett">
            Problemer som er «minst like vanskelige» som de hardeste i NP. Hvis
            du fant en polynom-algoritme for ett av dem, ville alle NP-problemer
            bli polynome (det er det berømte P=NP-spørsmålet). Eksempler:
            set cover, TSP, subset sum, SAT. Alle disse fire er det vi leker med.
          </Def>
          <Def term="Approksimasjon">
            «Vi finner ikke optimum raskt, men vi finner noe garantert nær».
            F.eks. en 1.5-approksimasjon for TSP betyr at svaret vi gir er
            høyst 1.5 ganger optimum.
          </Def>
          <Def term="H_n — harmonisk sum">
            Tallet <code>H_n = 1 + 1/2 + 1/3 + ... + 1/n</code>. For store n
            er <code>H_n ≈ ln(n)</code> (naturlig logaritme). Dukker opp som
            greedy-garantien for set cover.
          </Def>
          ...
          <Def term="Trekant-ulikheten">
            For tre punkter A, B, C: avstanden A→C er aldri lengre enn å gå
            A→B→C. Holder for vanlige avstander (Euklid, taxi-cab).
          </Def>
        </dl>
      </div>
      ...
      <div className="rounded-xl border border-border bg-card p-4">
        ...
        <ol className="list-decimal pl-5 space-y-1.5 text-muted-foreground">
          <li>
            <strong className="text-foreground">Set cover (modul 1)</strong> —
            enkleste greedy: «ta den som dekker flest udekkete». Bygger
            intuisjonen.
          </li>
          <li>
            <strong className="text-foreground">TSP (modul 2)</strong> —
            samme idé («ta nærmeste neste») men på et harderre problem.
            Sammenligning med brute force gjør NP-treghet konkret.
          </li>
          <li>
            <strong className="text-foreground">Knapsack (modul 3)</strong> —
            her bryter greedy. Introduserer DP (dynamisk programmering) som
            sammenligning.
          </li>
          <li>
            <strong className="text-foreground">P vs NP (modul 4)</strong> —
            samler det hele: greedy/DP fungerer fordi de fleste NP-problemer
            har approksimasjons-utveier. Subset sum-puslespill viser
            «verifiser vs finn».
          </li>
        </ol>
        <div className="mt-3 flex gap-2">
          <Button size="sm" onClick={() => onPick("setcover")}>Start på modul 1 →</Button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div>
      <div className="mb-3 rounded-lg border border-border bg-muted/30 p-3 text-xs text-muted-foreground">
        <strong className="text-foreground">Oppgaven:</strong> Vi har 8 byer i Norge.
        Hver radio-stasjon (S1–S6) dekker en delmengde av byene. Vi vil dekke
        ALLE 8 byer med <em>færrest mulige</em> stasjoner. Dette er klassisk{" "}
        <strong className="text-foreground">set cover</strong> — å dekke en
        «universemengde» med færrest «sett».
        <div className="mt-2">
          <strong className="text-foreground">Greedy-regelen vi bruker:</strong>{" "}
          i hver runde, velg stasjonen som dekker flest <em>udekkete</em> byer
          akkurat nå.
        </div>
      </div>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        ...
        <div className="rounded-xl border border-border bg-card p-3 text-xs">
          ...
          <div className="space-y-1.5">
            ...
            <Row label="Greedy ferdig på" value={`${steps[steps.length - 1].picked.length} stasjoner`} />
            <Row label="Optimum (brute force, 2⁶ = 64 forsøk)" value={`${optimum.count} stasjoner (${optimum.sets.join(", ")})`} />
            <div className="text-[11px] text-muted-foreground pt-2 space-y-1">
              <div>
                <strong className="text-foreground">Hva er garantien?</strong>{" "}
                For set cover bevises det at greedy alltid bruker høyst{" "}
                <code>H<sub>n</sub></code> ganger så mange sett som optimum,
                der <code>n</code> er antall byer (her 8) og{" "}
                <code>H<sub>n</sub> = 1 + 1/2 + 1/3 + ... + 1/n</code>.
              </div>
              <div>
                For <code>n = 8</code>: <code>H<sub>8</sub> ≈ 2.72</code>. For
                store n vokser <code>H<sub>n</sub></code> som{" "}
                <code>ln(n)</code> (naturlig logaritme). Det er ingen kjent
                hurtigere algoritme — set cover er <em>NP-hardt</em> (se «0.
                Start her» for definisjon).
              </div>
            </div>
          </div>
        </div>
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto">
      ...
      {step.picked.map((sid) => {
        ...
        return (
          <g key={sid}>
            <circle cx={cx} cy={cy} r={r} className="fill-brand/10 stroke-brand/40" strokeWidth={1.5} strokeDasharray="3 3" />
            <text x={cx} y={cy - r - 4} textAnchor="middle" className="fill-brand text-[10px] font-mono">
              {s.label}
            </text>
          </g>
        );
      ...
      {step.candidate && (() => {
        ...
        const r = Math.max(...cities.map((c) => Math.hypot(c.x - cx, c.y - cy))) + 18;
        return (
          <circle cx={cx} cy={cy} r={r} className="fill-amber-500/10 stroke-amber-500" strokeWidth={2} strokeDasharray="5 3" />
        );
      })()}
      {/* Byer */}
      {SC_CITIES.map((c) => {
        ...
        return (
          <g key={c.id}>
            <circle
              ...
              strokeWidth={2}
            />
            <text x={c.x} y={c.y + 22} textAnchor="middle" className="fill-foreground text-[10px] font-medium">
              {c.id}
            </text>
          </g>
        );
  ...
  return (
    <div>
      <div className="mb-3 rounded-lg border border-border bg-muted/30 p-3 text-xs text-muted-foreground">
        ...
        <strong className="text-foreground">TSP = Travelling Salesperson Problem</strong>:
        en selger må besøke alle byene én gang og returnere til start. Hva er
        det <em>korteste</em> mulige toget?
        <div className="mt-2">
          <strong className="text-foreground">To strategier her:</strong>
          <ul className="list-disc pl-5 mt-1 space-y-0.5">
            <li>
              <strong className="text-foreground">Nærmeste-nabo (greedy, blå):</strong>{" "}
              start på by 0, ta alltid nærmeste ubesøkte by. Rask:{" "}
              <code>O(n²)</code> — for hver av n byer sjekker vi avstand til alle andre.
            </li>
            <li>
              <strong className="text-foreground">Brute force (grønn):</strong>{" "}
              prøv ALLE mulige byrekkefølger og velg den korteste. Det er{" "}
              <code>(n−1)!</code> rekkefølger (vi fikserer startbyen). For
              n = 7 er det 720, for n = 9 er det 40 320, for n = 12 er det
      ...
      <div className="rounded-xl border border-border bg-card overflow-hidden">
        <div className="bg-muted/30 px-4 py-2 text-xs text-muted-foreground border-b border-border flex justify-between items-center">
          <span>
            Klikk i kartet for å legge til en by. By 0 (gul) er alltid startbyen.
          </span>
        ...
        >
          ...
          {points.map((p, i) => (
            <g key={p.id}>
              <circle cx={p.x} cy={p.y} r={10} className={i === 0 ? "fill-amber-500 stroke-amber-600" : "fill-card stroke-foreground/60"} strokeWidth={2} />
              <text x={p.x} y={p.y + 4} textAnchor="middle" className="fill-foreground text-[10px] font-mono font-semibold">{p.id}</text>
            </g>
          ))}
        </svg>
      </div>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-3">
        <div className="rounded-xl border border-brand/40 bg-brand/5 p-3 text-xs">
          <div className="text-[10px] uppercase tracking-wider text-muted-foreground mb-1">Nærmeste-nabo</div>
          <div className="font-mono text-lg">{greedyLen.toFixed(0)}</div>
          <div className="text-[10px] text-muted-foreground mt-1">O(n²) — alltid grådig nærmeste.</div>
        </div>
        <div className="rounded-xl border border-success/40 bg-success/5 p-3 text-xs">
          <div className="text-[10px] uppercase tracking-wider text-muted-foreground mb-1">Optimum (brute force)</div>
        ...
        <div className="rounded-xl border border-border bg-card p-3 text-xs">
          <div className="text-[10px] uppercase tracking-wider text-muted-foreground mb-1">Greedy / optimum</div>
          <div className="font-mono text-lg">{(ratio * 100).toFixed(1)}%</div>
          <div className="text-[10px] text-muted-foreground mt-1">
            {ratio < 1.05 ? "Greedy traff perfekt." : ratio < 1.25 ? "Greedy er nær." : "Greedy bommer mye."}
          </div>
        </div>
      </div>

      <div className="mt-4 flex flex-wrap gap-2">
        <Button size="sm" variant={showOptimum ? "default" : "outline"} onClick={() => setShowOptimum((v) => !v)}>
          {showOptimum ? "Skjul optimum" : "Vis optimum"}
        </Button>
        <Button size="sm" variant="outline" onClick={() => setPoints(TSP_DEFAULT)} className="gap-1.5">
...
function knapsackGreedy(items: Item[], cap: number): { picked: string[]; value: number; weight: number } {
...
function knapsackDp(items: Item[], cap: number): { picked: string[]; value: number; weight: number } {
...
function KnapsackModule() {
  ...
  return (
    <div>
      <div className="mb-3 rounded-lg border border-border bg-muted/30 p-3 text-xs text-muted-foreground">
        <strong className="text-foreground">Oppgaven:</strong>{" "}
        <strong className="text-foreground">Knapsack (ryggsekk):</strong>{" "}
        du er en tyv med en sekk som tåler en viss vekt. Hver gjenstand har en
        vekt og en verdi. Hvilke skal du ta for å maksimere verdi uten å sprenge
        vekt-grensa?
        <div className="mt-2">
          <strong className="text-foreground">To strategier:</strong>
          <ul className="list-disc pl-5 mt-1 space-y-0.5">
            <li>
              <strong className="text-foreground">Greedy (verdi/vekt-ratio):</strong>{" "}
              regn ut verdi-per-kg for hver gjenstand. Ta den med høyest ratio
              først, så nest høyeste, så lenge sekken har plass.
              <code>O(n log n)</code>.
            </li>
            <li>
              <strong className="text-foreground">DP (dynamisk programmering):</strong>{" "}
              et systematisk oppslagsverk. For hver (gjenstand i, kapasitet c)
              regner vi ut «beste mulige verdi». Bygger en n×C-tabell der
              C er max kapasitet. Garantert optimum. <code>O(n·C)</code>.
            </li>
          </ul>
          <div className="mt-1">
            DP er ikke i NP-klassen for dette problemet: kjøretiden er
            «pseudo-polynom» (avhenger av tall-størrelsen C). For store C
            blir også DP tregt — derfor vurderer vi fortsatt greedy.
          </div>
        </div>
      </div>
      <div className="rounded-xl border border-border bg-card p-4">
            ...
            {items.map((it) => {
              ...
              return (
                <tr key={it.id} className="border-t border-border">
                  ...
                  <td className="py-1.5 px-2 text-right font-mono">{(it.value / it.weight).toFixed(2)}</td>
...
function generateSubsetSum(n: number, target: number): { nums: number[]; solution: number[] } {
  ...
  const newSol = idx
    .map((i, newI) => (solIdx.has(i) ? newI : -1))
    .filter((x) => x >= 0);
...
function PvsNpModule() {
  ...
  return (
    <div>
      <div className="mb-3 rounded-lg border border-border bg-muted/30 p-3 text-xs text-muted-foreground">
        <strong className="text-foreground">Oppgaven:</strong>{" "}
        <strong className="text-foreground">Subset Sum (delmengde-sum):</strong>{" "}
        gitt en liste tall, finn en delmengde av dem som summerer til et bestemt
        mål. Eksempel: tall <code>[3, 7, 2, 8]</code>, mål <code>10</code> →
        delmengden <code>[8, 2]</code>.
        <div className="mt-2">
          Subset Sum er <strong className="text-foreground">NP-komplett</strong>{" "}
          (se «0. Start her» — det betyr: ingen kjent rask algoritme, men hvis
          noen gir deg svaret er det raskt å sjekke).
        </div>
        <div className="mt-2">
          <strong className="text-foreground">Hva vi prøver å vise:</strong>
          <ul className="list-disc pl-5 mt-1 space-y-0.5">
            <li>
              <strong className="text-foreground">Verifisere = lett:</strong>{" "}
              hvis du velger noen tall, kan vi summere dem og sjekke om de
              treffer målet i <code>O(n)</code>.
            </li>
            <li>
              <strong className="text-foreground">Finne = vanskelig:</strong>{" "}
              det er <code>2ⁿ</code> mulige delmengder (hver av de n tallene
              er enten med eller ikke). Brute force prøver alle. Dra n-slideren
              under og se hvordan tiden eksploderer.
            </li>
          </ul>
        </div>
      </div>
      <div className="rounded-xl border border-border bg-card p-4">
        <div className="text-sm mb-3">
          ...
          <span className="font-mono text-brand font-semibold">{target}</span>.
          Klikk på tall for å velge/avvelge.
        </div>
        ...
        <div className="text-sm flex items-center gap-3">
          <span>
            Sum:{" "}
            <span className={`font-mono font-semibold ${userSum === target ? "text-success" : userSum > target ? "text-destructive" : "text-foreground"}`}>
              {userSum}
            </span>{" "}
            / {target}
          </span>
          {userSum === target && (
            <span className="text-success text-xs">✓ Verifisert på O(n)</span>
          )}
        </div>
      </div>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        <div className="rounded-xl border border-border bg-card p-3 text-xs space-y-2">
          ...
          <p className="text-muted-foreground">
            Hvis noen gir deg et svar (en delmengde), tar det <code className="font-mono">O(n)</code> å summere
            og sjekke om det er riktig. Det er det som gjør subset sum til <strong>NP</strong>: «kan
            verifiseres i polynom tid».
          </p>
        </div>
        <div className="rounded-xl border border-border bg-card p-3 text-xs space-y-2">
          <div className="text-[10px] uppercase tracking-wider text-muted-foreground">
          ...
          <p className="text-muted-foreground">
            Å finne svaret krever (i verste fall) å sjekke alle <code className="font-mono">2ⁿ</code> delmengder.
            Det er ingen kjent polynom-algoritme. Det er det som gjør subset sum <strong>NP-hardt</strong>.
          </p>
          <Button size="sm" variant="outline" onClick={runBruteForce} disabled={bruteRunning || n > 25}>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2">
        ...
        <input
          ...
          className="max-w-xs"
        />
        <span className="font-mono text-xs tabular-nums">n={n} → 2ⁿ = {(1 << n).toLocaleString()}</span>
        <Button size="sm" variant="outline" onClick={() => regenerate()}>
          Nytt puslespill
        </Button>
        <Button size="sm" variant={showSol ? "default" : "outline"} onClick={() => setShowSol((v) => !v)}>
          {showSol ? "Skjul fasit" : "Vis fasit"}
        </Button>
      </div>

      <div className="mt-3 text-xs text-muted-foreground rounded-lg border border-border bg-card p-3">
        Legg merke til: når du øker n med 1, dobles bruteforce-tiden. Verifisering vokser
        bare lineært. <em>Hvis</em> P = NP ville løsing også vært polynom — men ingen har klart
        å bevise det på 50+ år. Det er fortsatt det største åpne problemet i CS.
      </div>
    </div>
  );
}
...
function Controls({ step, setStep, max }: { step: number; setStep: (n: number) => void; max: number }) {
  return (
    <div className="mt-4 flex flex-wrap items-center gap-2">
      ...
      <span className="text-xs text-muted-foreground">Steg {step + 1} / {max + 1}</span>
  ...
  return (
    <section className="mt-10 space-y-3 text-sm">
      ...
      <ul className="list-disc pl-5 space-y-2 text-muted-foreground">
        <li>
          <strong className="text-foreground">Greedy</strong> er strategien
          «alltid velg det som ser best ut nå». Fungerer perfekt for noen
          problemer (set cover gir <code>H<sub>n</sub></code>-garanti), bryter
          for andre (knapsack kan tape masse).
        </li>
        <li>
          <strong className="text-foreground">NP-komplett</strong> = problemer
          der vi ikke kjenner noen polynom-algoritme, men kan verifisere svar
          raskt. Hvis du finner én rask løser for ETT av dem (set cover, TSP,
          subset sum, SAT, ...), har du løst dem alle samtidig. Det berømte
          «P = NP?»-spørsmålet.
        </li>
        <li>
          <strong className="text-foreground">Approksimasjon</strong> = «vi
          finner ikke optimum, men noe garantert nær»:
          <ul className="list-disc pl-5 mt-1 space-y-1">
            <li>
              <strong className="text-foreground">For set cover:</strong>{" "}
              greedy gir høyst <code>H<sub>n</sub> ≈ ln(n)</code> ganger
              optimum.
            </li>
            <li>
              <strong className="text-foreground">For TSP der trekant-ulikheten gjelder</strong>{" "}
              (vanlige avstander der A→C ≤ A→B + B→C):{" "}
              <strong className="text-foreground">Christofides-algoritmen</strong>{" "}
              (Nicos Christofides, 1976) gir garantert høyst 1.5 × optimum.
              Bygger på minimum-spennende-tre + perfekt matching av
              odd-gradnoder.
            </li>
            <li>
              <strong className="text-foreground">For knapsack:</strong>{" "}
              det finnes en <strong className="text-foreground">FPTAS</strong>{" "}
              — «Fully Polynomial-Time Approximation Scheme». Du velger et ε
              (f.eks. 0.01 = 1% feil) og får et svar som er garantert innenfor
              (1−ε) × optimum, i polynom tid både i n OG 1/ε. Praktisk talt så
              nær du vil, så raskt du vil.
            </li>
          </ul>
        </li>
        <li>
          <strong className="text-foreground">Når du møter et nytt problem</strong>{" "}
          som ligner (schedulering, ressursvelging, lagvalg) — sjekk om det er
          kjent NP-hardt. Da slipper du å lete etter en perfekt algoritme som
          ikke finnes; finn en god approksimasjon i stedet.
        </li>
      </ul>
    </section>
  );
